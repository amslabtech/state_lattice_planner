...
TEST(MotionModelTest, GenerateTrajectory)
{
  ...
  mm.generate_trajectory(0.01, MotionModelDiffDrive::ControlParams(vel, omega),
                         trajectory);
...
TEST(TrajectoryGeneratorFunctionTest, GenerateOptimizedTrajectory)
{
  ...
  MotionModelDiffDrive::ControlParams init_params(
      init_v, MotionModelDiffDrive::AngularVelocityParams(0, 0, 0.5, 5));
  Eigen::Vector3d goal(5, 1, 1);
  MotionModelDiffDrive::Trajectory trajectory;
  std::cout << "generate optimized trajectory" << std::endl;
  double cost = tg.generate_optimized_trajectory(goal, init_params, 0.05, 1e-1,
                                                 5, output, trajectory);
...
TEST(TrajectoryGeneratorFunctionTest, GenerateNotOptimizedTrajectory)
{
  ...
  MotionModelDiffDrive::ControlParams init_params(
      init_v, MotionModelDiffDrive::AngularVelocityParams(
                  -0.8, 0, 0, goal.segment(0, 2).norm()));
  MotionModelDiffDrive::Trajectory trajectory;
  std::cout << "generate optimized trajectory" << std::endl;
  double cost = tg.generate_optimized_trajectory(goal, init_params, 1e-1, 1e-1,
...
TEST(MotionModelTest, GeenrateTrajectoryToBack)
{
  ...
  MotionModelDiffDrive::AngularVelocityParams omega(0.0, 0.0, 0.0, -5);
  MotionModelDiffDrive::Trajectory trajectory;
  mm.generate_trajectory(0.1, MotionModelDiffDrive::ControlParams(vel, omega),
                         trajectory);
...
// sharp curve test
TEST(TrajectoryGeneratorFunctionTest,
     GenerateOptimizedTrajectoryWithSharpCurve)
{
  TrajectoryGeneratorDiffDrive tg;
  tg.set_motion_param(1.0, 2.0, 1.0, 11.6, 0.125, 0.5);
  MotionModelDiffDrive::ControlParams output;
  ...
  MotionModelDiffDrive::ControlParams init_params(
      init_v, MotionModelDiffDrive::AngularVelocityParams(
                  0.0, 0.5, 0.0, goal.segment(0, 2).norm()));
  MotionModelDiffDrive::Trajectory trajectory;
  std::cout << "generate optimized trajectory" << std::endl;
  double cost = tg.generate_optimized_trajectory(goal, init_params, 0.1, 1e-1,
                                                 100, output, trajectory);
  std::cout << "trajecotry.back():" << std::endl;
  std::cout << trajectory.trajectory.back() << std::endl;
  std::cout << "cost: " << cost << std::endl;
