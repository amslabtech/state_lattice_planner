  ...
  for (int i = 0; i < N_YAW; i++)
  {
    for (int j = 0; j < N_Y; j++)
    {
      for (int k = 0; k < N_X; k++)
      {
        ...
        state << MIN_X + DELTA_X * k, -MAX_Y + DELTA_Y * j,
            -MAX_YAW + DELTA_YAW * i;
        if (state(0) == 0.0)
        {
          continue;
      ...
      for (auto state : states)
      {
        ...
        if (use_existing_lookup_table)
        {
          LookupTableUtils::get_optimized_param_from_lookup_table(
              lookup_table, state, v0, k0, optimized_param);
        }
        else
        {
        ...
        MotionModelDiffDrive::VelocityParams init_v(
            v0, MAX_ACCELERATION, target_velocity, target_velocity,
            MAX_ACCELERATION);
        MotionModelDiffDrive::ControlParams init(
            init_v, MotionModelDiffDrive::AngularVelocityParams(
                        k0, optimized_param.omega.km, optimized_param.omega.kf,
                        optimized_param.omega.sf));
        ...
        // std::cout << init.vel.v0 << ", " << init.vel.a0 << ", " <<
        // init.vel.vt << ", " << init.vel.af << ", " << init.vel.vf <<
        // std::endl; std::cout << init.omega.k0 << ", " << init.omega.km << ",
        // " << init.omega.kf << ", " << init.omega.sf << std::endl;
        double cost = tg.generate_optimized_trajectory(state, init, 1e-1, 1e-1,
                                                       100, output, trajectory);
        if (cost > 0)
        {
          std::cout << "\033[032msuccessfully optimized\033[0m" << std::endl;
          std::stringstream data;
          data << v0 << "," << k0 << "," << trajectory.trajectory.back()(0)
               << "," << trajectory.trajectory.back()(1) << ","
               << trajectory.trajectory.back()(2) << "," << output.omega.km
               << "," << output.omega.kf << "," << output.omega.sf << "\n";
          output_data += data.str();
          success_count++;
        }
        else
        {
          std::cout << "\033[031mfailed to optimize trajectory\033[0m"
                    << std::endl;
        }
      }
    }
  }
  int state_num = ((MAX_V - MIN_V) / DELTA_V) * 2 * (MAX_KAPPA / DELTA_KAPPA) *
                  states.size();
  std::cout << "success rate: " << success_count << " / " << state_num << " = "
            << static_cast<double>(success_count) / state_num << std::endl;
  return output_data;
}
...
void LookupTableGenerator::save(std::string& data)
{
  ...
  if (ofs)
  {
    ofs << data;
    ofs.close();
    std::cout << "lookup table saved as " << LOOKUP_TABLE_FILE_NAME
              << std::endl;
    exit(0);
  }
  else
